...
def main():
    ...
    img_cat_performance = df_completed.groupby('image_cluster_id')['final_reward'].agg(['mean', 'count']).reset_index()
    ...
    # <-- UPDATED column name here
    snd_cat_performance = df_completed.groupby('sound_cluster_id')['final_reward'].agg(['mean', 'count']).reset_index()
    ...
    df_completed.loc[:, 'reward_rolling_avg'] = df_completed['final_reward'].rolling(window=20, min_periods=1).mean()
    ax3.plot(df_completed.index, df_completed['final_reward'], 'o-', label='Per-Trial Reward', color='lightgray', markersize=4, alpha=0.7)
